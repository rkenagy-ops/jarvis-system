...
def _worker() -> None:
    ...
    _ib = IB()
    _connect_fails = 0  # consecutive connection failures — drives backoff
    while True:
        try:
        ...
        try:
            ...
            if not _ib.isConnected() or _ib_port != p:
                if _ib.isConnected():
                    try:
                        ...
                        pass
                # Backoff: 0s, 1s, 2s, 4s … up to 30s between reconnect attempts.
                if _connect_fails > 0:
                    wait = min(2 ** (_connect_fails - 1), 30)
                    _log.debug("ibkr reconnect backoff %ds (fail #%d)", wait, _connect_fails)
                    time.sleep(wait)
                _log.debug("ibkr connecting port=%d clientId=%d", p, cid)
                _ib.connect(host(), p, clientId=cid, timeout=6)
                _ib_port = p
                _connect_fails = 0
                _log.info("ibkr connected port=%d (%s)", p, PORTS.get(p, "unknown"))
                try:
            ...
            _log.warning("ibkr job error: %s: %s", type(exc).__name__, str(exc)[:200])
            box["e"] = exc
            _connect_fails += 1
            try:
                if _ib is not None and _ib.isConnected():
