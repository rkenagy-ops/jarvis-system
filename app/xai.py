...
def probe(*, force: bool = False) -> dict[str, Any]:
    ...
    with _probe_lock:
        if not force and _probe["ok"] is not None and now - _probe["checked"] < 90:
            return dict(_probe)  # return a snapshot so callers can't mutate shared state
    ...
    try:
        ...
        if resp.status_code == 200:
            update = {"ok": True, "reason": "ready", "checked": now}
        elif resp.status_code in {401, 403}:
            update = {"ok": False, "reason": "credits_or_auth", "checked": now}
        else:
            # Unknown — let Grok try; free fallback still catches errors.
            update = {"ok": True, "reason": f"http_{resp.status_code}", "checked": now}
    except Exception:
        update = {"ok": False, "reason": "network", "checked": now}
    with _probe_lock:
