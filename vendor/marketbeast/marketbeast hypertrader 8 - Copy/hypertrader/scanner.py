...
class StockScanner:
    ...
    def analyze(self, symbol: str, df: pd.DataFrame) -> Dict:
        ...
        if total_score > 0:
            score = bullish_score / total_score
        else:
            score = 0.5
...
def find_best_options(scanner: StockScanner, top_n: int = 10, target_dte: int = 7, direction_filter: str = None):
    ...
    for i, symbol in enumerate(FULL_MARKET):
        if (i + 1) % 50 == 0:
            print(f"  Scanned {i+1}/{total} symbols...")
        
        df = scanner.fetch_data(symbol)
        if df is not None and len(df) >= 20:
            try:
                analysis = scanner.analyze(symbol, df)
                if analysis['score'] >= 0.55:
                    all_results.append(analysis)
            except:
                pass
    
    print(f"  Found {len(all_results)} symbols with score >= 0.55")
...
def print_best_options(candidates: List[Dict]):
    ...
    if not candidates:
        print(f"\n{C.YELLOW}No suitable options opportunities found.{C.END}")
        return
